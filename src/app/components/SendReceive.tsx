...
export function SendReceive() {
  ...
  useEffect(() => {
    if (!address) return;
    try { setSavedTokens(JSON.parse(localStorage.getItem(`arc-wallet-tokens-${chainId}`) || "[]")); } catch { setSavedTokens([]); }
  }, [address, chainId]);

  const explorer = active.explorerUrl;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const entry = { hash, chainId, type: token === "native" ? "native" : "token", asset: token === "native" ? active.symbol : token.symbol, recipient, amount, createdAt: new Date().toISOString() };
      localStorage.setItem(key, JSON.stringify([entry, ...existing.filter((x: any) => x.hash !== hash)].slice(0, 100)));
  ...
  const submit = async () => {
    ...
    if (!Number.isFinite(value) || value <= 0) return setMessage("Enter a valid amount.");
    try {
  ...
  return <section className="mb-8 rounded-2xl border border-[#ffffff10] bg-[#0c1020] p-6">
    ...
    {tab === "send" ? <div className="max-w-xl space-y-4">
      ...
      <button onClick={submit} disabled={busy} className="w-full rounded-xl bg-[#63caff] py-3 font-black text-[#060810] disabled:opacity-50">{busy ? "Waiting for wallet…" : "Review & Send"}</button>
      ...
      {hash && <p className="text-xs text-[#00ffa3] font-mono">Submitted · <a className="underline" target="_blank" rel="noreferrer" href={`${explorer}/tx/${hash}`}>{hash.slice(0, 12)}…</a></p>}
