...
export function SendReceive() {
  ...
  const submit = async () => {
    ...
    try {
      if (token === "native") {
        sendTransaction({ to: recipient as `0x${string}`, value: parseUnits(amount, active.decimals) });
      } else {
        writeContract({ address: token.address, abi: erc20Abi, functionName: "transfer", args: [recipient as `0x${string}`, parseUnits(amount, token.decimals)] });
      }
